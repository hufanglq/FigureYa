...
a, Pairwise genomic and immune distances between every two tumour regions from the same tumour are compared (lung adenocarcinoma, P = 3.5 × 10−4 , n = 217;lung squamous cell carcinoma, P = 0.002, n = 186).
...
library(GSVA) # 用于基因集变异分析 (Load the GSVA package for gene set variation analysis)
library(ClassDiscovery) # 用于外部聚类分析 (Load the ClassDiscovery package for external clustering analysis)
library(vegan) # 用于生态与二元变量的距离计算 (Load the vegan package for ecological and binary variable distance calculations)

Sys.setenv(LANGUAGE = "en") # 设置环境语言为英文，以便显示英文报错信息 (Set environment language to English for English error messages)
options(stringsAsFactors = FALSE) # 禁止字符串自动转换为因子类型，避免数据处理中的意外转换 (Disable automatic conversion of strings to factors)
```
